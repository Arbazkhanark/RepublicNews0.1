...
// Helper function to extract plain text from HTML/markdown
...
// Helper function to truncate text with "Read More" option
const TruncatedText = ({ 
  text, 
  maxLength = 150,
  language = "en",
  className = "",
  showFullInModal = false
}: { 
  text: string; 
  maxLength?: number;
  language?: "en" | "hi";
  className?: string;
  showFullInModal?: boolean;
}) => {
  ...
  const displayText = isExpanded || !needsTruncation ? plainText : `${plainText.substring(0, maxLength)}...`;
  
  // For showing full content in modal
  const renderFullContentModal = () => {
    if (!showFullModal) return null;
    
    // Create a more readable version with paragraphs
    ...
    return (
      <Dialog open={showFullModal} onOpenChange={setShowFullModal}>
        <DialogContent className="sm:max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {language === "hi" ? "पूरा विवरण" : "Full Description"}
            </DialogTitle>
          </DialogHeader>
            ...
            {formattedText.map((paragraph, index) => {
              ...
              if (paragraph.startsWith('#')) {
                ...
                return (
                  <HeadingTag 
                    key={index} 
                    className={`font-bold ${headingLevel === 1 ? 'text-2xl' : headingLevel === 2 ? 'text-xl' : 'text-lg'} mt-4 mb-2`}
                  >
                    {headingText}
                  </HeadingTag>
                );
  ...
  return (
    <>
      <div className={`text-gray-700 ${className}`}>
        {displayText}
        {needsTruncation && !showFullInModal && (
          <>
            <button
        ...
        {needsTruncation && showFullInModal && (
          <button
            ...
            className="ml-2 text-blue-600 hover:text-blue-800 text-sm font-medium"
          >
            {language === "hi" ? "पूरा विवरण देखें" : "View Full Description"}
          </button>
        )}
      </div>
      
      {showFullInModal && renderFullContentModal()}
    </>
  );
};

// Helper function to format text with basic markdown/HTML support for truncated view
const formatTruncatedText = (text: string, maxLength: number = 150) => {
  if (!text) return "";
          ...
          <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded-r">
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <TruncatedText 
                  text={language === "hi" ? bookmark.factCheckHi : bookmark.factCheck} 
                  maxLength={200}
                  language={language}
                />
              </div>
    ...
    <>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "CollectionPage",
            "name": language === "hi" ? "फेक न्यूज डिबंकर" : "Fake News Debunker",
            ...
            "mainEntity": {
              "@type": "ItemList",
              "itemListElement": fakeNewsData.slice(0, 10).map((item, index) => ({
                "@type": "Article",
                "position": index + 1,
                "url": `https://republicmirror.com/fake-news/${item._id}`,
                "headline": language === "hi" ? item.titleHi : item.title,
                "datePublished": item.debunkedAt,
                "author": {
                  "@type": "Organization",
                  "name": "Republic Mirror Fact-Check Team"
                }
              }))
            }
          })
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="container mx-auto px-4 py-8" ref={containerRef}>
                              ...
                              <Card className="overflow-hidden hover:shadow-lg transition-shadow duration-300">
                                ...
                                <CardContent className="space-y-6">
                                  ...
                                  <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r">
                                    <div className="flex items-start gap-2">
                                      ...
                                      <div>
                                        ...
                                        <TruncatedText 
                                          text={language === "hi" ? newsItem.fakeClaimHi : newsItem.fakeClaim} 
                                          maxLength={200}
                                          language={language}
                                  ...
                                  <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded-r">
                                    <div className="flex items-start gap-2">
                                      ...
                                      <div>
                                        ...
                                        <TruncatedText 
                                          text={language === "hi" ? newsItem.factCheckHi : newsItem.factCheck} 
                                          maxLength={250}
                                          language={language}
                                          className="text-green-700"
                                          showFullInModal={true}
                                        />
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div>
                                    ...
                                    <TruncatedText 
                                      text={language === "hi" ? newsItem.explanationHi : newsItem.explanation} 
                                      maxLength={200}
                                      language={language}
          ...
          <div className="mt-8">
            <GoogleAdSense
              adSlot="728x90_fake_news_bottom_page"
              adFormat="horizontal"
              fullWidthResponsive={true}
              className="w-full"
            />
