...
export default function OpinionPage() {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(opinion =>
        getTitle(opinion).toLowerCase().includes(searchTerm.toLowerCase()) ||
        cleanContentForPreview(getContent(opinion), 500).toLowerCase().includes(searchTerm.toLowerCase()) ||
        opinion.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
    
    setFilteredOpinions(filtered);
  }, [opinions, searchTerm, topicFilter, language]);
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Error fetching data:", error);
      toast.error(language === 'hi' ? "समाचार लोड करने में विफल" : "Failed to load news articles");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLike = async (opinionId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(language === 'hi' ? "पसंद किया!" : "Liked!");
        fetchData();
      }
    } catch (error) {
      console.error("Error liking:", error);
      toast.error(language === 'hi' ? "पसंद करने में विफल" : "Failed to like");
    }
  };
  ...
  const handleDislike = async (opinionId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(language === 'hi' ? "नापसंद किया!" : "Disliked!");
        fetchData();
      }
    } catch (error) {
      console.error("Error disliking:", error);
      toast.error(language === 'hi' ? "नापसंद करने में विफल" : "Failed to dislike");
    }
  };
  ...
  const handleShare = async (opinion: Opinion, e: React.MouseEvent) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: getTitle(opinion),
          text: cleanContentForPreview(getContent(opinion), 100),
          url: `${window.location.origin}/opinion/${opinion._id}`,
        });
      } catch (error) {
        console.log('Sharing cancelled');
      ...
      navigator.clipboard.writeText(`${window.location.origin}/opinion/${opinion._id}`);
      toast.success(language === 'hi' ? "लिंक कॉपी किया गया!" : "Link copied to clipboard!");
    }
  };
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center">
              ...
              <h1 className="text-2xl font-bold text-gray-900">
                {language === 'hi' ? 'राय और विचार' : 'OPINIONS'}
              </h1>
            </div>
            ...
            <div className="hidden md:flex items-center gap-4">
              <div className="relative w-64">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
                <Input
                  placeholder={language === 'hi' ? "राय खोजें..." : "Search opinions..."}
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="pl-10"
                />
              ...
              <div className="text-sm text-gray-600">
                {language === 'hi' ? `${filteredOpinions.length} राय` : `${filteredOpinions.length} opinions`}
              </div>
            </div>
          </div>
          ...
          <nav className="hidden md:block border-t">
            <div className="flex items-center justify-between py-2">
              ...
              <div className="text-sm text-gray-500">
                <Clock className="inline h-4 w-4 mr-1" />
                {new Date().toLocaleTimeString(language === 'hi' ? 'hi-IN' : 'en-US', { hour: '2-digit', minute: '2-digit' })}
              </div>
            </div>
          </nav>
        </div>
          ...
          <div className="mb-8 border-b pb-8">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2">
                ...
                <h1 className="text-4xl font-bold mb-4 leading-tight text-gray-900">
                  {getTitle(filteredOpinions[0])}
                </h1>
                
                <div className="flex items-center text-gray-600 mb-6">
                  <span className="text-sm">
                    {language === 'hi' ? 'लेखक: ' : 'By '}
                    {typeof filteredOpinions[0].authorId === 'object' ? filteredOpinions[0].authorId.name : getAuthor(filteredOpinions[0].authorId)?.name || "Staff Reporter"}
                  </span>
                  <span className="mx-2">•</span>
                  <span className="text-sm">{formatDate(filteredOpinions[0].createdAt)}</span>
                  <span className="mx-2">•</span>
                ...
                {/* Extract and display first image */}
                {(() => {
                  const firstImage = extractFirstImage(getContent(filteredOpinions[0]));
                  if (firstImage) {
                    return (
                      <div className="mb-6">
                        <img
                          src={firstImage}
                          alt={getTitle(filteredOpinions[0])}
                          className="w-full h-96 object-cover rounded-lg"
                        />
                        <p className="text-sm text-gray-500 mt-2">
                          {typeof filteredOpinions[0].authorId === 'object' ? filteredOpinions[0].authorId.name : getAuthor(filteredOpinions[0].authorId)?.name || "Photo"} / 
                          {language === 'hi' ? 'समाचारदैनिक' : 'NewsDaily'}
                        </p>
                      </div>
                    );
                  }
                ...
                <div className="prose max-w-none mb-6">
                  <div className="text-lg leading-relaxed text-gray-700" dangerouslySetInnerHTML={{
                    __html: formatPreviewContent(cleanContentForPreview(getContent(filteredOpinions[0]), 400))
                  }} />
                </div>
              ...
              {/* Top Stories Sidebar */}
              <div className="border-l pl-8">
                <h2 className="text-xl font-bold mb-4 pb-2 border-b text-gray-900">
                    ...
                    >
                      <div className="flex items-start">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-600" dangerouslySetInnerHTML={{
                            __html: formatPreviewContent(cleanContentForPreview(getContent(opinion), 80))
                          }} />
                          <div className="flex items-center text-xs text-gray-500 mt-2">
                            <Clock className="h-3 w-3 mr-1" />
                            {getTimeSince(opinion.createdAt)}
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6 border-b pb-2">
            <h2 className="text-2xl font-bold text-gray-900">
              {language === 'hi' ? 'नवीनतम राय' : 'LATEST OPINIONS'}
            </h2>
            <div className="flex gap-2">
              {getUniqueTopics().slice(1, 6).map((topic) => (
                <Button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredOpinions.slice(1).map((opinion, index) => {
              const author = typeof opinion.authorId === 'object' ? opinion.authorId : getAuthor(opinion.authorId);
              const imageFromContent = extractFirstImage(getContent(opinion));
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="text-gray-600 text-sm mb-3 line-clamp-3" dangerouslySetInnerHTML={{
                      __html: formatPreviewContent(cleanContentForPreview(getContent(opinion), 120))
                    }} />
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500 mb-3">
                      <div className="flex items-center">
                        ...
                        <span>{author?.name || (language === 'hi' ? 'स्टाफ रिपोर्टर' : 'Staff Reporter')}</span>
                      </div>
                      <div className="flex items-center">
                        <Clock className="h-3 w-3 mr-1" />
                        {getTimeSince(opinion.createdAt)}
              ...
              <p className="text-gray-600 mb-6">
                {searchTerm || topicFilter !== "all"
                  ? language === 'hi' 
                    ...
                    : "Try adjusting your search or filter"
                  : language === 'hi'
                    ? "अभी तक कोई स्वीकृत लेख उपलब्ध नहीं है"
                    : "No approved articles available yet"}
              </p>
              {(searchTerm || topicFilter !== "all") && (
                <Button 
        ...
        {filteredOpinions.length > 5 && (
          <div className="mb-8 bg-gray-50 p-6">
            <h2 className="text-2xl font-bold mb-6 pb-2 border-b text-gray-900">
              {language === 'hi' ? 'संपादक की पसंद' : 'EDITOR\'S PICKS'}
            </h2>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {filteredOpinions.slice(1, 4).map((opinion) => (
                <div 
                ...
                >
                  <div className="flex items-center mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {language === 'hi' ? 'लेखक: ' : 'By '}
                        {typeof opinion.authorId === 'object' ? opinion.authorId.name : getAuthor(opinion.authorId)?.name || 
                          (language === 'hi' ? 'संपादकीय टीम' : 'Editorial Team')}
                      </p>
                    </div>
                  </div>
                  <div className="text-gray-700 mb-3" dangerouslySetInnerHTML={{
                    __html: formatPreviewContent(cleanContentForPreview(getContent(opinion), 150))
                  }} />
                  <Button variant="ghost" className="text-red-600 hover:text-red-700 p-0">
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <p className="text-gray-400 mb-4">
                {language === 'hi' 
                  ? 'दुनिया भर के नवीनतम समाचार, विश्लेषण और राय के लिए आपका विश्वसनीय स्रोत।'
                  : 'Your trusted source for the latest news, analysis, and opinion pieces from around the world.'}
              </p>
              <div className="flex gap-2">
                <Button variant="outline" className="border-gray-700 text-white hover:bg-gray-800 rounded-none">
            ...
            <div>
              ...
              <ul className="space-y-2 text-gray-400">
                <li><Link href="/" className="hover:text-white">{language === 'hi' ? 'होम' : 'Home'}</Link></li>
                <li><Link href="/opinions" className="hover:text-white">{language === 'hi' ? 'सभी राय' : 'All Opinions'}</Link></li>
              </ul>
            </div>
            ...
            <div>
              ...
              <p className="text-gray-400 mb-4">
                {language === 'hi' 
                  ? 'दैनिक अपडेट के लिए हमारे न्यूज़लेटर की सदस्यता लें।'
                  : 'Subscribe to our newsletter for daily updates.'}
              </p>
              <div className="flex">
                <Input 
          ...
          <div className="flex flex-col md:flex-row justify-between items-center text-sm text-gray-400">
            <p>© {new Date().getFullYear()} NewsDaily. {language === 'hi' ? 'सर्वाधिकार सुरक्षित।' : 'All rights reserved.'}</p>
            <div className="flex gap-6 mt-4 md:mt-0">
              <a href="#" className="hover:text-white">{language === 'hi' ? 'गोपनीयता नीति' : 'Privacy Policy'}</a>
              <a href="#" className="hover:text-white">{language === 'hi' ? 'सेवा की शर्तें' : 'Terms of Service'}</a>
              <a href="#" className="hover:text-white">{language === 'hi' ? 'कुकी नीति' : 'Cookie Policy'}</a>
            </div>
          </div>
        </div>
      </footer>
